...
import { DEFAULT_STREAM_URL, WhepReceiver, sampleMetrics, validateStreamUrl, videoSample, type VideoSample } from "./stream-client";
...
export function StreamPreview() {
  ...
  const connect = async (event: FormEvent) => {
    ...
    disconnect();
    try { validateStreamUrl(url, window.location.origin); }
    ...
    dispose.current = () => {
      active = false; clearTimeout(timer); clearTimeout(firstFrameTimeout);
    ...
    try {
      await receiver.start(url, window.location.origin);
  ...
  return (
    <section className="stream-panel" aria-labelledby="stream-title">
      <div className="stream-heading"><div><p className="eyebrow">VÍDEO INDEPENDENTE</p><h2 id="stream-title">Transmissão da câmera</h2><p>Vídeo contínuo, sem aguardar a análise de pessoas.</p></div><span className="stream-target">Meta: 720p · 30 FPS</span></div>
      <form className="stream-form" onSubmit={(event) => void connect(event)}>
        <label htmlFor="stream-address">Endereço do stream WHEP<input id="stream-address" value={url} onChange={(event) => setUrl(event.target.value)} disabled={busy} spellCheck={false} autoComplete="off" required aria-describedby="stream-help" /></label>
      ...
      <p id="stream-help" className="stream-help">Conexão local nesta etapa. Inicie o servidor de mídia e o transmissor no computador. Uma URL RTSP ou MJPEG não é um endereço WHEP.</p>
      <div className={`stream-screen stream-${status}`}>
        <video ref={video} autoPlay muted playsInline aria-label="Transmissão da câmera sem análise de IA" />
      ...
      <div className="stream-note">IA não conectada a este vídeo. Nenhuma contagem ou caixa de detecção é estimada aqui; o fluxo não grava imagens.</div>
    </section>
  );
}
